...
const writeEmitter = (writable) => ({
  emit: (msg) => writable.write(JSON.stringify([msg]) + '\n')
})
...
ImageStore.prototype.create = function (req, res, callback) {
  ...
  this._store[id] = {
    ...
    Config: {
      Labels: req && req.query && req.query.Labels && JSON.parse(req.query.Labels)
    }
  }
